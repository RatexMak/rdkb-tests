...
public class BroadBandSyndicatePartnerTest extends BroadBandWebUiBaseTest {
	 ...
	 * <ol>
	 * <li>Verify whether DCM_LOG_SERVER_URLis present in /etc/dcm.properties and it
	 * points to endpoint.</li>
	 * <li>Reboot the device</li>
	 * <li>Wait till the device is up and Validate Process</li>
	 * <li>Verify whether all must have fields are populated in searchResults sent
	 * to telemetry server from dcmscript.log</li>
	 * <li>Verify whether device populates \"erouterIpv4\" details as part of Data
	 * sent to Telemetry Server.</li>
	 * <li>Verify whether device populates \"erouterIpv6\" details as part of Data
	 * sent to Telemetry Server.</li>
	 * <li>Verify whether device populates \"PartnerId\" details as part of Data
	 * sent to Telemetry Server</li>
	 * </ol>
	 * 
	 * @param Dut
	...
	public void testToVerifyErouterIPsAndPartnerIP(Dut settop) {
		...
		LOGGER.info(
				"1. Verify whether DCM_LOG_SERVER_URLis present in /etc/dcm.properties and it points to DCM endpoint.");
		LOGGER.info("2. Reboot the device");
		LOGGER.info("3. Wait till the device is up and Validate Process");
		LOGGER.info(
				"4. Verify whether all must have fields are populated in searchResults sent to telemetry server from dcmscript.log");
		LOGGER.info(
				"5. Verify whether device populates \"erouterIpv4\" details as part of  Data sent to Telemetry Server.");
		LOGGER.info(
				"6. Verify whether device populates \"erouterIpv6\" details as part of  Data sent to Telemetry Server.");
		LOGGER.info(
				"7. Verify whether device populates \"PartnerId\" details as part of  Data sent to Telemetry Server");
		...
		try {
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info(
					"STEP 1: DESCRIPTION : Verify whether DCM_LOG_SERVER_URLis present in /etc/dcm.properties and it points to DCM endpoint.");
			LOGGER.info(
					"STEP 1: ACTION : Execute COMMANDS : rm -rf /nvram/.DCMSettings.conf  /tmp/DCMSettings.conf /nvram/dcm.properties");
			LOGGER.info(
					"STEP 1: EXPECTED : DCM endpoint (DCM_LOG_SERVER_URL) should present in /etc/dcm.properties and it endpoint with HTTPS");
			LOGGER.info("**********************************************************************************");
...
//			}
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info(
					"STEP 4: DESCRIPTION : Verify whether all must have fields are populated in searchResults sent to telemetry server from dcmscript.log");
			LOGGER.info(
					"STEP 4: ACTION : Execute cat dcmscript.log and verify all mandatory fields are there in rdklogs");
			LOGGER.info(
					"STEP 4: EXPECTED : Device should populate required telemetry data and upload to telemetry server.");
			LOGGER.info("**********************************************************************************");

			boolean isTelemetry2Enabled = BroadBandWebPaUtils.getAndVerifyWebpaValueInPolledDuration(settop, tapEnv,
			...
			if (BroadbandPropertyFileHandler.isSplunkEnabled() && isTelemetry2Enabled) {

				telemetryJsonPayLoad = BroadBandTelemetryUtils.getTelemetryDataPayloadGeneratedFromSplunk(settop,
						tapEnv);
			} else {
				telemetryJsonPayLoad = BroadBandTelemetryUtils.getTelemetryDataPayloadGeneratedFromDcmScriptLog(settop,
						tapEnv);
			}

			JSONObject telemetryData = BroadBandTelemetryUtils.getTelemetryDataPayLoadAsJsonFormat(settop, tapEnv,
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info(
					"STEP 5: DESCRIPTION : Verify whether device populates erouterIpv4 details as part of  Data sent to Telemetry Server and cross validate with erouter0 interface");
			LOGGER.info(
					"STEP 5: ACTION : Execute cat dcmscript.log , ifconfig erouter0 and verify the erouterIpv4 in rdklogs with erouter0 interface");
			LOGGER.info(
					"STEP 5: EXPECTED : Device should populates erouterIpv4 details as part of  Data sent to Telemetry Server");
			LOGGER.info("**********************************************************************************");
			...
			String ifConfigErouterResponse = tapEnv.executeCommandUsingSsh(settop,
					BroadBandTestConstants.IFCONFIG_EROUTER);
			String expectedErouterIpv4Address = CommonMethods.patternFinder(ifConfigErouterResponse,
					BroadBandTestConstants.INET_V4_ADDRESS_PATTERN);
			status = CommonMethods.isNotNull(expectedErouterIpv4Address)
					&& expectedErouterIpv4Address.equalsIgnoreCase(actualErouterIpv4Address);
			if (status) {
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info(
					"STEP 6: DESCRIPTION : Verify whether device populates erouterIpv6 details as part of  Data sent to Telemetry Server and cross validate with erouter0 interface");
			LOGGER.info(
					"STEP 6: ACTION : Execute cat dcmscript.log , ifconfig erouter0 and verify the erouterIpv6 in rdklogs with erouter0 interface");
			LOGGER.info(
					"STEP 6: EXPECTED : Device should populates erouterIpv6 details as part of  Data sent to Telemetry Server");
			LOGGER.info("**********************************************************************************");
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info(
					"STEP 7: DESCRIPTION : Verify whether device populates PartnerId details as part of  Data sent to Telemetry Server and validate with webpa ");
			LOGGER.info(
					"STEP 7: ACTION : Execute cat dcmscript.log , Param : Device.DeviceInfo.X_RDKCENTRAL-COM_Syndication.PartnerId and verify the PartnerId in rdklogs with webpa");
			LOGGER.info(
					"STEP 7: EXPECTED : Device should populates PartnerId details as part of  Data sent to Telemetry Server");
			LOGGER.info("**********************************************************************************");
			status = BroadBandTelemetryUtils.verifyTelemetryPayLoadParamameter(settop, tapEnv, telemetryData,
					TelemetryData.PARTNER_ID);
			if (status) {
