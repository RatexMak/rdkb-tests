...
public class BroadBandSnmpWareHouseOIDTest extends AutomaticsTestBase {
	 ...
	 * <li>Verify set for the SNMP MIB 1.3.6.1.2.1.69.1.3.7.0 for upgrade
	 * server</li>
	...
	public void testSnmpSetOnWareHouseOIDs(Dut device) {
		...
		boolean isRPiDevice = DeviceModeHandler.isRPIDevice(device);
		...
		try {
			...
			/**
			 * PRECONDITION 2 : SET THE CHANNEL SELECTION MODE TO MANUAL FOR 2.4 AND 5 GHz
			 * USING WEBPA
			 */
			errorMessage = null;
			status = false;
			...
			if (!isRPiDevice) {
				...
				for (BROADBAND_WAREHOUSE_SNMP_LIST warehouseSnmp : BROADBAND_WAREHOUSE_SNMP_LIST.values()) {
					if (((warehouseSnmp.getMode().equals(SNMP_MODE.SET)
							...
							&& !(CommonMethods.patternMatcher(warehouseSnmp.getInfo(),
									BroadBandTestConstants.FACTORY_RESET)
									|| CommonMethods.patternMatcher(warehouseSnmp.getInfo(), "wireless ssid enable 5g")
									|| CommonMethods.patternMatcher(warehouseSnmp.getInfo(),
			...
			if (snmpListForThisTest != null && !snmpListForThisTest.isEmpty()) {
				...
				for (BROADBAND_WAREHOUSE_SNMP_LIST warehouseSnmp : snmpListForThisTest) {
					...
					/**
					 * Step-7:Set defaultChannel2Ghz value as channel value for 2Ghz Step-8:Set
					 * defaultChannel5Ghz value as channel value for 5Ghz
					 **/
	...
	/**
	 * Method to perform apply settings after setting channel value for 2.4Ghz and
	 * 5ghz
	 * 
	 ...
	 * protocol</li>
	 * <li>Verify set for the SNMP MIB 1.3.6.1.2.1.69.1.3.3.0 for upgrade
	 * status</li>
	 * <li>Verify set for the SNMP MIB 1.3.6.1.2.1.69.1.3.2.0 for upgrade file</li>
	 * <li>Verify set for the SNMP MIB 1.3.6.1.4.1.17270.50.2.1.1.1008.0 for upgrade
	 * with reset</li>
	 * </ol>
	 * 
	 * @author anandam.s
	...
	/**
	 * Verify SNMP get operation for warehouse DOCSIS OIDs and cross-verify the same
	 * with its corresponding TR-181 param using WebPA
	 * <ol>
	 ...
	 * <li>Verify SNMP MIB for firmware update and factory reset is performed with
	 * only a single reset.</li>
	 ...
	 * <li>Verify SNMP MIB docsDevSwServerAddressType is set to IPv4 address
	 * type.</li>
	...
	public void testECMFwUpgradeAndFactoryResetOIDsSequenceOnWarehouse(Dut device) {
		...
		try {
			...
			/**
			 * Method to set Download Protocol, Server Address Type, Server address,
			 * Firmware image for HTTP Ipv6 download
			 */
			...
			/**
			 * Method to set Download Protocol, Server Address Type, Server address,
			 * Firmware image for TFTP Ipv4 download
			 */
	 ...
	 * 
	 * @param tapEnv     AutomaticsTapApi device
	 * @param device     Dut device
	 * @param firmware   Firmware to be upgraded
	 ...
	 * download.</li>
	 * <li>Verify SNMP code download has not started.</li>
	 * <li>Verify SNMP MIB FACTORY_RESET_DEVICE is set for Factory Resetting the
	 * device.</li>
	 * <li>Post-Condition 1: Verify reverting original build of the device.</li>
	 * </ol>
	 * 
	 ...
	 * <li>Step 5: Validate default Mac Address and password length after factory
	 * reset</li>
	...
	public void testToVerifyRdkbWareHouseSequence(Dut device) {
		...
		try {
			...
			LOGGER.info("STEP 2: ACTION : Execute snmp get command with uid 1.3.6.1.2.1.1.3.0");
			...
			LOGGER.info("STEP 3: ACTION : Execute snmp set command using uid 1.3.6.1.4.1.17270.50.2.1.1.1002.0 i 1");
			...
			LOGGER.info("STEP 4: ACTION : Execute snmp get command using uid 1.3.6.1.2.1.1.3.0");
			...
			LOGGER.info("STEP 10: ACTION : Execute snmp command snmpget using OID 1.3.6.1.2.1.1");
			...
			LOGGER.info("STEP 19: ACTION : Execute snmp command snmp set using OID 1.3.6.1.4.1.17270.50.2.1.1.1002.");
