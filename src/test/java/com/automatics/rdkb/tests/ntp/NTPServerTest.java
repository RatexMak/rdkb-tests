...
/**
 * Test class to that contains test cases which verify Moca diagnostics using WEBPA for TR181 params
 * 
 * @author susheela c
 * 
 */
public class NTPServerTest extends AutomaticsTestBase {
    ...
    /**
     * 
     * Test case to Validate if device synchronizes times when a time drift is created and NTP server enabled
     * 
     * <p>
     * STEPS:
     * </p>
     ...
     * <li>STEP 6: Validate the device time after waiting for 15 mins if synchronized</li>
     * <li>STEP 7: Verify if NTP server url change to a incorrect url is successful</li>
     * <li>STEP 8: Check the logs to validate if the device starts polling fallback default NTP server url</li>
     * </ol>
     * 
     * @param device
	...
	try {
	    ...
	    try {
	    String ntpServerUrl = AutomaticsPropertyUtility.getProperty(BroadBandPropertyKeyConstants.PROPKEY_FOR_NTPHOST);
		...
		String ntpServerUrl = AutomaticsPropertyUtility.getProperty(BroadBandPropertyKeyConstants.PROPKEY_FOR_NTPHOST);
		status = NTPServerUtils.validateNTPServerEnableAndUrlInLogs(tapEnv, device,
				ntpServerUrl, deviceDateTime);
	    } catch (TestException exp) {
		status = false;
		errorMessage = exp.getMessage();
		LOGGER.error(errorMessage);
		...
		status = NTPServerUtils.validateNTPServerEnableAndUrlInLogs(tapEnv, device,
				ntpServerInvalidUrl, deviceDateTime);
	    } catch (TestException exp) {
		status = false;
		errorMessage = exp.getMessage();
		LOGGER.error(errorMessage);
