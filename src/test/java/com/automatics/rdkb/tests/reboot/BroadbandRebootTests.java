...
public class BroadbandRebootTests extends AutomaticsTestBase {
		...
		try {
			...
			if (isSupportedDevices || CommonMethods.isAtomSyncAvailable(device, tapEnv)) {
				...
				status = BroadBandWebPaUtils.setAndVerifyParameterValuesUsingWebPaorDmcli(device, tapEnv,
						...
						AutomaticsPropertyUtility.getProperty(BroadBandPropertyKeyConstants.PROP_KEY_BLASTER_PLANID),
						BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
				if (status) {
				...
				status = BroadBandWebPaUtils.setAndVerifyParameterValuesUsingWebPaorDmcli(device, tapEnv,
						...
						AutomaticsPropertyUtility.getProperty(BroadBandPropertyKeyConstants.PROP_KEY_BLASTER_DESTMAC),
						BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
				if (status) {
	...
	/**
	 * Test to verify the behavior of SNMP MIB
	 * DOCS-CABLE-DEVICE-MIB::docsDevResetNow(.1.3.6.1.2.1.69.1.1.3) using eCM IP
	 * Address. Device should not reboot if we set docsDevResetNow as 2 using SNMP
	 * command. If we set docsDevResetNow as 1, then device should reboot and comes
	 * up within 5 minutes.
	 * 
	 * <ol>
	 * <li>Step 1 : Verify Setting SNMP MIB
	 ...
	 * DOCS-CABLE-DEVICE-MIB::docsDevResetNow(.1.3.6.1.2.1.69.1.1.3) to 2.</li>
	 * <li>Step 3 : Verify Setting SNMP MIB
	...
	public void verifyDeviceRebootUsingDocsDevResetNowSnmpCommand(Dut device) {
		...
		try {
			...
			/*
			 * Due to large buffer size, it takes more time for searching in buffer. So
			 * clearing buffer and start buffering the trace.
			 */
	 ...
	 * <li>PRE CONDITION 2: Verify whether Selfheal process is Up and Running in the
	 * Device.</li>
	 * <li>Verify whether brlan0 is assigned properly with valid DHCPv4
	 * address.</li>
	 * <li>Verify whether brlan0 is assigned properly with valid DHCPv6
	 * address.</li>
	...
	public void testToverifyBrlan0interfaceReboot(Dut device) {
		...
		try {
			...
			if (BroadbandPropertyFileHandler.isIpv6Enabled()) {
				LOGGER.info("**********************************************************************************");
				LOGGER.info(
						"STEP 3: DESCRIPTION : Verify whether brlan0 is assigned properly with valid DHCPv6 address.");
				LOGGER.info(
			...
			LOGGER.info("**********************************************************************************");
			/**
			 * Tailing the logs continuously from /rdklogs/logs/SelfHeal.txt.0 log file to
			 * /nvram/sample.txt for AtomConsole devices
			 */
			/**
			 * Tailing the logs continuously from /rdklogs/logs/SelfHealAggressive.txt.0 log
			 * file to /nvram/sample.txt for other than AtomConsole devices
			 */
			...
			if (BroadbandPropertyFileHandler.isIpv6Enabled()) {
				LOGGER.info("**********************************************************************************");
				LOGGER.info(
						"STEP 8: DESCRIPTION : Verify whether brlan0 is assigned properly with valid DHCPv6 address.");
				LOGGER.info(
	 ...
	 * "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.Enable" using
	 * webpa</li>
	 * <li>Check the default value for the parameter
	 * "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.UpTime" using
	 * webpa</li>
	 * <li>Configure the values beyond the range(1 to 30).Values should be
	 * rejected.Execute less than 1</li>
	 * <li>Configure the values beyond the range(1 to 30).Values should be
	 * rejected.Execute more than 30</li>
	 * <li>Ensure previous values are set for parameter
	 * "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.UpTime" using
	 * webpa</li>
	 * <li>Enable the parameter
	 * "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.Enable" using
	 * webpa</li>
	 * <li>Check for log strings "AutoReboot Set param Enable value 1" and "sh
	 * /etc/ScheduleAutoReboot.sh 10 1&" in PAM.log.txt</li>
	 * <li>Ensure "/etc/AutoReboot.sh" is created in crontab</li>
	 * <li>Disable the parameter
	 * "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.Enable" using
	 * webpa</li>
	 * <li>Check for log strings "AutoReboot Set param Enable value 0" and "sh
	 * /etc/ScheduleAutoReboot.sh 0 0&" in PAM.log.txt</li>
	...
	/**
	 * Verify NMI mtdoops& SRAM support for BCM ARM processors to capture system
	 * state during watchdog reset
	 * <ol>
	 * 
	 ...
	 * <li>4. Verify Broadcom SRAM debug dump info exists
	 * /rdklogs/logs/resetinfo.txt.0</li>
	 * <li>5. Verify Broadcom SRAM debug dump info exists
	 * /nvram/6/reset_reason.log</li>
	 * <li>6. Verify pattern \"PREVIOUS_KERNEL_OOPS_DUMP\" is in
	 * /nvram/6/reset_reason.log</li>
	 * <li>7. Verify pattern \"PREVIOUS_KERNEL_OOPS_DUMP\" is in
	 * /rdklogs/logs/resetinfo.txt.0</li>
	 ...
	 * <li>12. Verify Broadcom SRAM debug dump info exists
	 * /rdklogs/logs/resetinfo.txt.0</li>
	 * <li>13. Verify Broadcom SRAM debug dump info exists
	 * /nvram/6/reset_reason.log</li>
	 * <li>14. Verify pattern \"PREVIOUS_KERNEL_OOPS_DUMP\" is in
	 * /nvram/6/reset_reason.log</li>
	 * <li>15. Verify pattern \"PREVIOUS_KERNEL_OOPS_DUMP\" is in
	 * /rdklogs/logs/resetinfo.txt.0</li>
	...
	/**
	 * Test case is created as part of Coverage Improvement based on the RDKB-4008
	 * [Performance] Add Device.DeviceInfo.X_RDKCENTRAL-COM_BootTime TR-181 object
	 *
	 * Test Case # 1: Verify Device.DeviceInfo.X_RDKCENTRAL-COM_BootTime TR-181
	 * parameter
	 *
	 * <p>
	 * STEPS:
	 * </p>
	 * <ol>
	 * <li>STEP 1: Verify BootTime using TR-181
	 * Parameter(Device.DeviceInfo.X_RDKCENTRAL-COM_BootTime)</li>
	 * </ol>
	 *
