...
public class BroadBandXconfCdlTest extends AutomaticsTestBase {
	...
	try {
		...
		/*
		 * status = BroadBandWebPaUtils.setAndVerifyParameterValuesUsingWebPaorDmcli(device, tapEnv,
		 * BroadBandWebPaConstants.WEBPA_COMMAND_MAINTENANCE_WINDOW_END_TIME, BroadBandTestConstants.CONSTANT_0,
     ...
     * <li>Code download request should reject with following message in xconf.txt.0 XCONF SCRIPT : Current image (
     * <current_image_name>) and Requested imgae (<current_image_name>) are same. No upgrade/downgrade required</li>
     * </p>
     * <p>
     * 
     * @param device
	...
	try {
		 ...
		 * POST CONDITION : VERIFY THAT DEVICE IS UPGRADED WITH THE PREVIOUS FIRMWARE VERSION.
		 */
		BroadBandCodeDownloadUtils.triggerPreviousCodeDownload(device, tapEnv, initialFirmwareVersion);
	    }
	    LOGGER.info("################### COMPLETED POST-CONFIGURATIONS ###################");
	}
    ...
    /**
     * Verify whether multiple udhcpc.script error messages getting printed in Consolelog.txt
     * <ol>
     * <li>Upgrade the device to latest build by using TR-181</li>
     * <li>Upgrade the device to latest build by using SNMP, IF CDL BY WEBPA FAILED</li>
     * <li>Verify whether /etc/udhcpc.script error messages are getting printed in the Consolelog.txt.0 file</li>
     ...
     * </p>
     * <ol>
     * <li>Step 1: Verify Triggered CDL Successful with latest firmware version through xconf</li>
     * <li>Step 2: Verify log information "DEVICE_INIT" is available in Consolelog.txt.0/ArmConsolelog.txt.0 log after
     * upgrade</li>
     * <li>Step 3: Verify log information "Firmware upgrade start time" is available in /rdklogs/logs/xconf.txt.0 log
     * after upgrade</li>
     * <li>Step 4: Verify log information "Firmware upgrade end time" is available in /rdklogs/logs/xconf.txt.0 log
     * after upgrade</li>
     * <li>Step 5: Verify the upgrade time via xconf</li>
     * <li>Step 6: Verify Triggered CDL Successful with previuos firmware version through xconf</li>
     * <li>Step 7: Verify log information "DEVICE_INIT" is available in Consolelog.txt.0/ArmConsolelog.txt.0 log after
     * downgrade</li>
     * <li>Step 8: Verify log information "Firmware upgrade start time" is available in /rdklogs/logs/xconf.txt.0 log
     * after downgrade</li>
     * <li>Step 9: Verify log information "Firmware upgrade end time" is available in /rdklogs/logs/xconf.txt.0 log
     * after downgrade</li>
     * <li>Step 10: Verify the downgrade time via xconf</li>
     * <li>POST CONDITION 1 : Verify that device is upgraded with the previous firmware version in #S6, if not try CDL
     * again</li>
     * </ol>
     * 
     * @param device
	...
	try {
	    ...
	    /**
	     * Step 1 : VERIFY TRIGGERED CDL SUCCESSFULL WITH LATEST FIRMWARE VERSION THROUGH XCONF
	     */
	    ...
	    /**
	     * Step 2 : VERIFY TRIGGERED CDL SUCCESSFULL WITH LATEST FIRMWARE VERSION THROUGH XCONF
	     */
	    stepNumber = 2;
	    stepNum = "S" + stepNumber;
	    status = false;
	    ...
	    /**
	     * Step 7 : VERIFY TRIGGERED CDL SUCCESSFULL WITH PREVIOUS FIRMWARE VERSION THROUGH XCONF
	     */
	    stepNumber = 7;
	    stepNum = "S" + stepNumber;
	    status = false;
	    ...
	    LOGGER.info("################### STARTING POST-CONFIGURATIONS ###################");
	    /**
	     * POST CONDITION : VERIFY THAT DEVICE IS UPGRADED WITH THE PREVIOUS FIRMWARE VERSION, IF NOT TRY CDL AGAIN
	     */
    ...
    /**
     * Test method used to validate the XCONF log information and calculate the upgrade/downgrade time
     * 
     * @param device
     *            instance of{@link Dut}
     * @param tapEnv
     ...
     * @param stepNumber
     *            Step Number
     * @param latestFirmwareVersion
     *            FirmwareVersion To CDL
     * @param cdlLogsForValidation
     *            CDL logs for validation
     * @param startTimeXconf
     *            start time for xconf cdl
     * @param endTimeXconf
     *            end time for xconf cdl
     * @param cdlMessage
     *            upgrade/downgrade
     * @Refactor Sruthi Santhosh
     */
	...
	/**
	 * SETP : VERIFY LOG INFORMATION "DEVICE_INIT" IS AVAILABLE IN CONSOLELOG.TXT.0/ARMCONSOLELOG.TXT.0
	 */
	stepNum = "S" + stepNumber;
	status = false;
	LOGGER.info("**********************************************************************************");
	...
	/**
	 * SETP : VERIFY LOG INFORMATION "FIRMWARE UPGRADE START TIME" IS AVAILABLE IN /RDKLOGS/LOGS/XCONF.TXT.0
	 */
	stepNumber++;
	stepNum = "S" + stepNumber;
	status = false;
	...
	/**
	 * SETP : VERIFY LOG INFORMATION "FIRMWARE UPGRADE END TIME" IS AVAILABLE IN XCONF.TXT.0
	 */
	stepNumber++;
	stepNum = "S" + stepNumber;
	status = false;
	...
	try {
	    ...
	    errorMessage = "Failed to verify /etc/" + firmwareDownloadScript + " 2 cron job running status";

	    LOGGER.info("**********************************************************************************");
	    LOGGER.info("STEP 5: DESCRIPTION : Verify \"/etc/" + firmwareDownloadScript
		    + " 2\" cdl trigger cron job is running or not");
	    LOGGER.info("STEP 5: ACTION : Execute \"crontab -l\" command and verify result");
	    LOGGER.info("STEP 5: EXPECTED : crontab -l command response must contain \"/etc/" + firmwareDownloadScript
		...
		if (status) {
		    LOGGER.info("SUCCESSFULLY VERIFIED /etc/" + firmwareDownloadScript
		...
		errorMessage = "It seems /etc/" + firmwareDownloadScript + " 2 cron job is not currently running";
		LOGGER.error(errorMessage);
	    }

	    if (status) {
		LOGGER.info("STEP 5: ACTUAL : SUCCESSFULLY VERIFIED /etc/" + firmwareDownloadScript
			+ " CRON JOB IS RUNNING");
	    } else {
	    ...
	    if (status) {
		LOGGER.info("POST-CONDITION " + postConStepNumber
			+ " ACTUAL : Successfully disabled periodic firmware upgrade.");
	    } else {
    ...
    /**
     * Test Case : Verify CPE is able to download and install the cert bundle for myrouter.io
     * <p>
     * STEPS:
     * </p>
     * <ol>
     * <li>PRE-CONDITION 1 : Verify swupdate.conf is configured with mock xconf server url</li>
     * <li>PRE-CONDITION 2 : Verify lighttpd process is running in the device</li>
     * <li>1: Configure dlCertBundle for DUT in mock xconf server</li>
     * <li>2: Reboot the device</li>
     * <li>3: Check whether dlCertBundle details were sent to xconf server via curl query</li>
     * <li>4: Check whether dlCertBundle details was received in the xconf response</li>
     * <li>5: Check whether xconf cdl script called /etc/rdm/rdmBundleMgr.sh to process bundle update</li>
     * <li>6: Check whether rdm package installation is successful</li>
     * <li>7: Verify Package installation successful log is occurred in /rdklogs/logs/webuiupdate.log</li>
     * <li>8: Verify version is same in installed json file for cert bundles configured in xconf server</li>
     * <li>9: Check the validity of the certificate installed is accurate for downloaded cert bundles</li>
     * <li>10: Verify lighttpd process is running in the device</li>
     * </ol>
     * 
     * @param device
     *            {@link Dut}
    ...
    @TestDetails(testUID = "TC-RDKB-XCONF_CERT_DWNLD-1001")
    public void testToVerifyXConfCertDownloableBundle(Dut device) {
	// Variable Declaration begins
	String testCaseId = "TC-RDKB-XCONF_CERT_DWNLD-001";
	String stepNum = null;
	boolean status = false;
	String response = null;
	String errorMessage = "";
	...
	// Variable Declaration Ends
	LOGGER.info("#######################################################################################");
	LOGGER.info("STARTING TEST CASE: TC-RDKB-XCONF_CERT_DWNLD-1001");
	LOGGER.info("TEST DESCRIPTION: Verify device is able to download and install the cert bundle for myrouter.io");
	LOGGER.info("PRE-CONDITION 1 : Verify swupdate.conf is configured with mock xconf server url");
	LOGGER.info("PRE-CONDITION 2 : Verify lighttpd process is running in the device");
	LOGGER.info("1: Configure dlCertBundle for DUT in mock xconf server");
	LOGGER.info("2: Reboot the device");
	LOGGER.info("3: Check whether dlCertBundle details were sent to xconf server via curl query");
	LOGGER.info("4: Check whether dlCertBundle details was received in the xconf response");
	LOGGER.info("5: Check whether xconf cdl script called /etc/rdm/rdmBundleMgr.sh to process bundle update");
	LOGGER.info("6: Check whether rdm package installation is successful");
	LOGGER.info("7: Verify Package installation successful log is occurred in /rdklogs/logs/webuiupdate.log");
	LOGGER.info("8: Verify version is same in installed json file for cert bundles configured in xconf server");
	LOGGER.info("9: Check the validity of the certificate installed is accurate for downloaded cert bundles");
	LOGGER.info("10: Verify lighttpd process is running in the device");
	LOGGER.info("#######################################################################################");
	...
	try {
	    ...
	    LOGGER.info("PRE-CONDITION STEPS");
	    /**
	     * PRE-CONDITION 1 : Configure automation server url in swupdate.conf
	     */
	    LOGGER.info("#######################################################################################");
	    LOGGER.info(
		    "PRE-CONDITION 1 : DESCRIPTION : Verify swupdate.conf is configured with mock xconf server url");
	    LOGGER.info(
		    "PRE-CONDITION 1 : ACTION : Execute grep command: grep -I <MOCK XCONF URL> /nvram/swupdate.conf");
	    LOGGER.info(
		    "PRE-CONDITION 1 : EXPECTED : /nvram/swupdate.conf must be configured with mock xconf server url");
	    LOGGER.info("#######################################################################################");
	    errorMessage = "Unable to configure mock server url in /nvram/swupdate.conf";
	    status = BroadBandTelemetryUtils.configureAndVerifySwupdateWithXconfUrl(device, tapEnv);

	    if (status) {
		LOGGER.info("PRE-CONDITION 1 : ACTUAL : swupdate.conf is configured with mock xconf server url");
	    } else {
	    ...
	    /**
	     * PRE-CONDITION 2 : Verifying lighttpd process is running in the device.
	     */
	    response = null;
	    status = false;
	    LOGGER.info("#######################################################################################");
	    LOGGER.info("PRE-CONDITION 2 : DESCRIPTION : Verify lighttpd process is running in the device");
	    LOGGER.info("PRE-CONDITION 2 : ACTION : Execute command:ps | grep -I lighttpd");
	    LOGGER.info("PRE-CONDITION 2 : EXPECTED : lighttpd process should be running in the device");
	    LOGGER.info("#######################################################################################");

	    errorMessage = "lighttpd process is not running in the device";
	    response = CommonMethods.getPidOfProcess(device, tapEnv, BroadBandTestConstants.PROCESS_NAME_LIGHTTPD);
	    status = CommonMethods.isNotNull(response);

	    if (status) {
		LOGGER.info("PRE-CONDITION 2 : ACTUAL : lighttpd process running in the device");
	    } else {
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Configure dlCertBundle for DUT in xconf server");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION :  dlCertBundle version detail should configured in xconf server using Rest Api call");
	    LOGGER.info("STEP " + stepNumber + " : EXPECTED : dlCertBundle version must be configurd in xconf server");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "Unable to configure dlCertBundle for the device in Xconf.";
	    JSONObject jsonObj = new JSONObject();
	    jsonObj.put(BroadBandTestConstants.CONSTANT_DLCERTBUNDLE,
		    BroadbandPropertyFileHandler.getWebuiCertBundleVersion());
	    try {
		XConfUtils.configureXconfDownloadFirmwareDetails(device,
			...
			false, BroadBandTestConstants.FIRMWARE_DOWNLOAD_PROTOCOL_HTTP,
			BroadBandTestConstants.EMPTY_STRING,
			AutomaticsTapApi.getSTBPropsValue(XconfConstants.PROP_KEY_XCONF_FIRMWARE_LOCATION),
			BroadBandTestConstants.CONSTANT_0, false, jsonObj);
		// Updating status to true as didn't get exception while configuring cert bundle details in xconf.
		status = true;
	    } catch (Exception e) {
		LOGGER.info("Exception occured while " + e);
		status = false;
	    }
		...
		tapEnv.waitTill(BroadBandTestConstants.ONE_MINUTE_IN_MILLIS);
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : Sucessfully configured dlcertbundle for the device in xconf server");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("STEP " + stepNumber + " : ACTION : Do webpa or sbin reboot in the device");
	    LOGGER.info("STEP " + stepNumber + " : EXPECTED : Device must be accessible after reboot");
	    LOGGER.info("***************************************************************************************");
	    ...
	    if (status) {
		LOGGER.info("STEP " + stepNumber + " : ACTUAL : Device is accessible after reboot");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Check whether dlCertBundle details were sent to xconf server via curl query");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION : Execute grep command: grep -I dlCertBundle /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : dlCertBundle details were sent to xconf server via curl query must occur in /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("***************************************************************************************");
	    tapEnv.waitTill(BroadBandTestConstants.FIVE_MINUTE_IN_MILLIS);
	    LOGGER.info("waiting for 5 mins:");
	    errorMessage = "dlCertBundle details is not sent to xconf server via curl query in /rdklogs/logs/xconf.txt.0";
	    response = CommonUtils.searchLogFiles(tapEnv, device, BroadBandTestConstants.CONSTANT_DLCERTBUNDLE,
		    BroadBandTestConstants.RDKLOGS_LOGS_XCONF_TXT_0, BroadBandTestConstants.THIRTEEN_MINUTE_IN_MILLIS,
		    BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);
	    status = CommonMethods.isGivenStringAvailableInCommandOutput(response, BroadBandTestConstants.CURL);
	    if (!status) {
		response = tapEnv.searchAndFindLineWithMatchingStringFromStart(device,
			BroadBandTestConstants.CONSTANT_DLCERTBUNDLE, BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);
		status = CommonMethods.isGivenStringAvailableInCommandOutput(response, BroadBandTestConstants.CURL);
	    }

	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : dlCertBundle details are sent to xconf server via curl query");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Check whether dlCertBundle details was received in the xconf response");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION : Execute grep command: grep -I dlCertBundle /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : Xconf server response of dlCertBundle details must occured in /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = BroadbandPropertyFileHandler.getWebuiCertBundleVersion()
		    + " Xconf server response of dlCertBundle details not occured in /rdklogs/logs/xconf.txt.0";
	    status = CommonMethods.isGivenStringAvailableInCommandOutput(response,
		    BroadbandPropertyFileHandler.getWebuiCertBundleVersion());
	    if (!status) {
		response = null;
		response = CommonUtils.searchLogFiles(tapEnv, device,
			BroadbandPropertyFileHandler.getWebuiCertBundleVersion(),
			BroadBandTestConstants.RDKLOGS_LOGS_XCONF_TXT_0, BroadBandTestConstants.EIGHT_MINUTE_IN_MILLIS,
			BroadBandTestConstants.FIFTEEN_SECONDS_IN_MILLIS);
		status = CommonMethods.isNotNull(response) && CommonMethods.isGivenStringAvailableInCommandOutput(
			response, BroadbandPropertyFileHandler.getWebuiCertBundleVersion());
	    }
	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : dlCertBundle details is received in the /rdklogs/logs/xconf.txt.0 from xconf response.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Check whether xconf cdl script called /etc/rdm/rdmBundleMgr.sh to process bundle update");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION : Execute grep command: grep -I BundleMgr /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : Xconf script calling rdmBundleMgr log should present in /rdklogs/logs/xconf.txt.0");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "Xconf script calling rdmBundleMgr log is not present in /rdklogs/logs/xconf.txt.0";
	    response = CommonUtils.searchLogFiles(tapEnv, device, BroadBandTraceConstants.LOG_MESSAGE_RDM_BUNDLE_MGR,
	    ...
	    status = CommonMethods.isGivenStringAvailableInCommandOutput(response,
		    BroadBandTraceConstants.LOG_MESSAGE_PROCESS_BUNDLE_UPDATE);
	    if (!status) {
		response = tapEnv.searchAndFindLineWithMatchingStringFromStart(device,
			BroadBandTraceConstants.LOG_MESSAGE_RDM_BUNDLE_MGR,
			BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);
		status = CommonMethods.isGivenStringAvailableInCommandOutput(response,
			BroadBandTraceConstants.LOG_MESSAGE_PROCESS_BUNDLE_UPDATE);
	    }
	    ...
	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : conf cdl script called /etc/rdm/rdmBundleMgr.sh to process bundle update.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    response = null;
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber + " : DESCRIPTION : Check whether rdm package installation is successful");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION :  Execute grep command: grep -I \"webui-cert-bundle\" /rdklogs/logs/rdm_status.log");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : package installed successfully log must present in /rdklogs/logs/rdm_status.log for all the cert bundles configured in xconf server");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "package installed successfully log is not present in /rdklogs/logs/rdm_status.log for the cert configured in xconf server";
	    response = CommonUtils.searchLogFiles(tapEnv, device,
		    BroadBandTraceConstants.LOG_MESSAGE_WEBUI_CERT_BUNDLE_FILE
			    + BroadBandTestConstants.SINGLE_SPACE_CHARACTER
			    + BroadBandTraceConstants.LOG_MESSAGE_PACKAGE_INSTALLED_SUCCESSFULLY,
		    BroadBandCommandConstants.CMD_GET_CABUNDLE_RDMSTATUS_LOG,
		    BroadBandTestConstants.EIGHT_MINUTE_IN_MILLIS, BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
	    status = CommonMethods.isNotNull(response);
	    if (!status) {
		response = CommonUtils.searchLogFiles(tapEnv, device,
			BroadBandTraceConstants.LOG_MESSAGE_WEBUI_CERT_BUNDLE_FILE,
			BroadBandCommandConstants.CMD_GET_CABUNDLE_RDMSTATUS_LOG,
			BroadBandTestConstants.TWO_MINUTE_IN_MILLIS, BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
		status = CommonMethods.isGivenStringAvailableInCommandOutput(response,
			BroadBandTraceConstants.LOG_MESSAGE_NO_UPDATE_REQUIRED)
			|| CommonMethods.isGivenStringAvailableInCommandOutput(response,
				BroadBandTraceConstants.LOG_MESSAGE_PACKAGE_INSTALLED_SUCCESSFULLY);
		if (!status) {
		    response = tapEnv.searchAndFindLineWithMatchingStringFromStart(device,
			    BroadBandTraceConstants.LOG_MESSAGE_WEBUI_CERT_BUNDLE_FILE,
	    ...
	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : Successful rdm package installation log is occured in /rdklogs/logs/rdm_status.log for the cert configured in xconf server.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Verify Package installation successful log is occurred in /rdklogs/logs/webuiupdate.log");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION :   Execute grep command: grep -I \"Package installed successfully\" /rdklogs/logs/webuiupdate.log");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : package installed successfully log must present in /rdklogs/logs/webuiupdate.log");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "package installed successfully log is not present in rdklogs/logs/webuiupdate.log for the cert configured in xconf server";
	    response = CommonUtils.searchLogFiles(tapEnv, device,
		    BroadBandTraceConstants.LOG_MESSAGE_PACKAGE_INSTALLED_SUCCESSFULLY,
		    BroadBandCommandConstants.FILE_GET_WEBUI_UPDATE_LOG, BroadBandTestConstants.FIVE_MINUTE_IN_MILLIS,
		    BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
	    status = CommonMethods.isNotNull(response);

	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : Successful rdm package installation log is occured in /rdklogs/logs/webuiupdate.log for the cert configured in xconf server.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Verify version is same in installed json file for cert bundles configured in xconf server");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION : Execute grep command: cat /nvram/etc/certs/tstwebuicert_package.json | grep version");
	    LOGGER.info("STEP " + stepNumber
		    + " : EXPECTED : Version is same in installed json file for cert bundles configured in xconf server");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "package installed successfully log is not present in /rdklogs/logs/rdm_status.log for the cert configured in xconf server";
	    response = CommonUtils.searchLogFiles(tapEnv, device, BroadBandTraceConstants.LOG_MESSAGE_VERSION,
		    BroadBandCommandConstants.FILE_GET_WEBUI_CERT_BUNDLE_PACKAGE,
		    BroadBandTestConstants.ONE_MINUTE_IN_MILLIS, BroadBandTestConstants.TEN_SECOND_IN_MILLIS);
	    String versionNumber = BroadbandPropertyFileHandler.getWebuiCertBundleVersion().replaceAll("[a-zA-Z-:]", "")
		    .trim();
	    LOGGER.info("the version number :" + versionNumber);
	    status = CommonMethods.patternMatcher(response, versionNumber);
	    LOGGER.info("status :" + status);

	    if (status) {
		LOGGER.info("STEP " + stepNumber
			+ " : ACTUAL : version is same in installed json file for cert bundles configured in xconf server.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	    ...
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber
		    + " : DESCRIPTION : Check the validity of the certificate installed is accurate for downloaded cert bundles");
	    LOGGER.info("STEP " + stepNumber
		    + " : ACTION : Execute command: 1)openssl x509 -noout -dates -in /nvram/certs/myrouter.io.cert.pem");
	    LOGGER.info("STEP " + stepNumber + " : EXPECTED : Certificate validity must be configured properly");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "package installed successfully log is not present in /rdklogs/logs/rdm_status.log for the cert configured in xconf server";
	    response = tapEnv.executeCommandInSettopBox(device,
		    BroadBandCommandConstants.CMD_TO_OPEN_MYROUTER_CERTFICATE
			    + BroadBandTestConstants.FILE_NVRAM_MYROUTER_IO_CERT_WEBUI);
	    status = CommonMethods.isGivenStringAvailableInCommandOutput(response,
		    BroadBandTestConstants.STRING_MYROUTER_CERTIFICATE_BEFORE_VALIDITY_DATE)
		    && CommonMethods.isGivenStringAvailableInCommandOutput(response,
			    BroadBandTestConstants.STRING_MYROUTER_CERTIFICATE_AFTER_VALIDITY_DATE);
	    ...
	    response = null;
	    LOGGER.info("***************************************************************************************");
	    LOGGER.info("STEP " + stepNumber + " : DESCRIPTION : Verify lighttpd process is running in the device");
	    LOGGER.info("STEP " + stepNumber + " : ACTION : Execute command: ps | grep -I lighttpd");
	    LOGGER.info("STEP " + stepNumber + " : EXPECTED : lighttpd process should be running in the device");
	    LOGGER.info("***************************************************************************************");
	    errorMessage = "lighttpd process is not running in the device";
	    response = CommonMethods.getPidOfProcess(device, tapEnv, BroadBandTestConstants.PROCESS_NAME_LIGHTTPD);
	    status = CommonMethods.isNotNull(response);
	    ...
	    if (status) {
		LOGGER.info("STEP " + stepNumber + " : ACTUAL : Lighttpd process is running in the device.");
	    } else {
		LOGGER.error("STEP " + stepNumber + " : ACTUAL : " + errorMessage);
	    }
	    LOGGER.info("**********************************************************************************");
	...
	LOGGER.info("ENDING TEST CASE: TC-RDKB-XCONF_CERT_DWNLD-1001");
    }
}
