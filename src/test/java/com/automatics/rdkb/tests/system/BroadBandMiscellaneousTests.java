...
public class BroadBandMiscellaneousTests extends AutomaticsTestBase {
	 ...
	 * <li>Create procanalyzerconfig.ini under nvram with dynamic values and verify
	 * file procanalyzerconfig.ini</li>
	 * <li>Create processes.list under nvram anf verify /nvram/processes.list</li>
	 * <li>Check the default value for the parameter
	 * "Device.SelfHeal.X_RDK_CPUProcAnalyzer_Enable" using webpa</li>
	 * <li>Verify UPLOAD_LOGS_VAL_DCM value is set to true using sysevent</li>
	 * <li>Enable the parameter "Device.SelfHeal.X_RDK_CPUProcAnalyzer_Enable" using
	 * webpa</li>
	 * <li>Verify the process is up for /usr/bin/cpuprocanalyzer</li>
	 * <li>Verify the default values under /etc/procanalyzerconfig.ini is
	 * "FEATURE.CPUPROCANALYZER.SLEEP.SECS = 60
	 * FEATURE.CPUPROCANALYZER.TIMETORUN.SECS = 600 FEATURE.CPUPROCANALYZER.DYNAMIC
	 * = 1"</li>
	 ...
	 * <li>Verify captured data in cpuprocanalyzer is not persistent on post
	 * reboot.</li>
	 * 
	 ...
	 * <li>PRE-CONDITION-1 Get the number of fans installed in the device using
	 * TR-181 FanNumberOfEntries</li>
	 * <li>PRE-CONDITION-2 Set the value of
	 * Device.DeviceInfo.Thermal.Fan.{i}.MaxOverride to TRUE using webpa</li>
	 * <li>1. Verify the default value of Device.Thermal.Fan.{i}.Status using
	 * webpa</li>
	 * <li>2. Verify the default value of Device.Thermal.Fan.{i}.Speed using
	 * webpa</li>
	 * <li>3. Verify the default value of Device.Thermal.Fan.{i}.RotorLock using
	 * webpa</li>
	 * <li>4. Verify the default value of Device.Thermal.Fan.{i}.MaxOverride using
	 * webpa</li>
	 * <li>5. Set the value of Device.Thermal.Fan.{i}.MaxOverride to TRUE using
	 * webpa</li>
	 * <li>6. Verify the value of Device.Thermal.Fan.{i}.MaxOverride using
	 * webpa</li>
	 * <li>7. Reboot the device</li>
	 * <li>8. Verify Device.Thermal.Fan.{i}.Status is not persisted after
	 * reboot</li>
	 * <li>9. Verify Device.Thermal.Fan.{i}.Speed is not persisted after reboot</li>
	 * <li>10. Verify Device.Thermal.Fan.{i}.RotorLock is not persisted after
	 * reboot</li>
	 * <li>11. Verify the value of Device.Thermal.Fan.{i}.MaxOverride once the
	 * device comes online after reboot</li>
	 * <li>POST-CONDITION-1 Set the value of
	 * Device.DeviceInfo.Thermal.Fan.{i}.MaxOverride to False using webpa</li>
	 * 
	...
	/**
	 *
	 * Test Case : Verify CPE on-boarding process logging in the already onboarded
	 * device
	 *
	 * <p>
	 * STEPS:
	 ...
	 * <li>Step 4 : Verify that get for
	 * \"Device.DeviceInfo.X_RDKCENTRAL-COM_OnBoarding_State\" parameter is
	 * "OnBoarded"</li>
	 * <li>Step 5 : Verify "sysevent get snmp-onboard-reboot" is EMPTY</li>
	 * <li>Step 6 : Trigger DOCSIS SNMP REBOOT</li>
	 * <li>Step 7 : Once the device comes up, verify the LastRebootReason as
	 * Docsis_SNMP_Reboot</li>
	 * <li>Step 8 : Verify that \"OnBoardingLog*.txt.0\" file is not created after
	 * reboot</li>
	 * <li>Step 9 : Trigger DOCSIS SNMP REBOOT</li>
	 * <li>Step 10 : Once the device comes up, verify the LastRebootReason as
	 * Docsis_SNMP_Reboot</li>
	...
	public void testToVerifyOnboardingProcessLoggingInTheAlreadyOnboardedDevice(Dut device) {
		...
		try {
			...
			LOGGER.info("STEP " + stepNumber + ": DESCRIPTION : Verify that "
					+ BroadBandCommandConstants.FILE_ETC_ONBOARD_LOGGING_ENABLE + " file is created ");
			LOGGER.info("STEP " + stepNumber + ": ACTION : Execute the following command: ls -al "
			...
			LOGGER.info("**********************************************************************************");
			LOGGER.info("STEP " + stepNumber + ": DESCRIPTION : Verify that "
					+ BroadBandCommandConstants.FILE_NVRAM_DEVICE_ONBOARDED + " file is created ");
			LOGGER.info("STEP " + stepNumber + ": ACTION : Execute the following command: ls -al "
					+ BroadBandCommandConstants.FILE_NVRAM_DEVICE_ONBOARDED);
			...
			if (isOnboardLogsFileExist) {
				onboardLogsCreatedTime = tapEnv.executeCommandUsingSsh(device,
						BroadBandCommandConstants.CMD_GET_ONBOARD_LOGS_TIME);
			}
			if (status) {
			...
			/**
			 * Step 7: ONCE THE DEVICE COMES UP, VERIFY THE LASTREBOOTREASON AS
			 * DOCSIS_SNMP_REBOOT
			 */
			stepNumber++;
			step = "S" + stepNumber;
			...
			status = !CommonUtils.isFileExists(device, tapEnv,
					BroadBandCommandConstants.FILE_NVRAM_ONBOARDLOGS_ONBOARDINGLOG)
					|| response.equalsIgnoreCase(onboardLogsCreatedTime);
			...
			/**
			 * Step 10 : ONCE THE DEVICE COMES UP, VERIFY THE LASTREBOOTREASON AS
			 * DOCSIS_SNMP_REBOOT
			 */
			stepNumber++;
			step = "S" + stepNumber;
	 ...
	 * </p>
	 * <ol>
	 * <li>PRE CONDITION 1 : Check telemetry process is running in the device, If
	 * not enable the telemetry & verify</li>
	 * <li>PRE CONDITION 2 : Set SelfHeal trigger interval to 4 minutes</li>
	 * <li>1. Verify nonroot support blocklist parameter value is null or No
	 * blocklisted process by default</li>
	 ...
	 * <li>7. Verify the default capabilities for Telemetry2.0 are defined in JSON
	 * config file.</li>
	 * <li>8. Enable nonroot support blocklist feature using RFC</li>
	 * <li>9. Verify nonroot support blocklist feature parameter is Telemetry2.0
	 * using RFC</li>
	 ...
	 * <li>POST CONDITION 1 : Set Null value to Non-root Blocklist parameter through
	 * RFC</li>
	 ...
	 * <li>1. Verify Accountid is set in syscfg.db file using syscfg get command and
	 * cross validate via dmcli</li>
	 * <li>2. Verify RFC xconf query has &accountID=unknown in the curl request</li>
	 * <li>3.Verify CDL xconf query has &accountID=<accountID> in the curl
	 * request</li>
	 * <li>4. Verify DCM xconf query has &accountID=<accountID> in the curl
	 * request</li>
	 * <li>5. Reboot the device and verify AccountId persist</li>
	 * <li>6. Remove the AccounID by setting Unknown to syscfg.db file using syscfg
	 * set command</li>
	 * <li>7. Configure mock xconf url in swupdate.conf</li>
	 * <li>8. Restart rfc-config.service to fetch the RFC settings from the mock
	 * xconf</li>
	 * <li>9. Reboot the device</li>
	 * <li>10. Verify RFC xconf query has &accountID=unknown in the curl
	 * request</li>
	 * <li>11. Verify CDL xconf query has &accountID=unknown in the curl
	 * request</li>
	 ...
	 * <li>14. Restart rfc-config.service to fetch the RFC settings from the mock
	 * xconf</li>
	 * <li>15. Reboot the device</li>
	 * <li>16. Verify the AccounID is set in syscfg.db file using syscfg set command
	 * and cross validate via dmcli</li>
	 * <li>17. Verify RFC xconf query has &accountID=unknown in the curl
	 * request</li>
	 * <li>18. Verify CDL xconf query has &accountID=<accountID> in the curl
	 * request</li>
	 * <li>19. Verify DCM xconf query has &accountID=<accountID> in the curl
	 * request</li>
	 * <li>20. Verify invalid AccoundID with special characters is not set in the
	 * xconf query's via dmcli command</li>
	 * <li>21. Verify invalid AccoundID with special characters that is set is not
	 * present in the xconf query's via dmcli command</li>
	 * <li>22. Verify invalid AccoundID with more than 32 characters is not set in
	 * the xconf query's via dmcli command</li>
	 * <li>23. Verify invalid AccoundID with more than 32 characters that is set is
	 * not present in the xconf query's via dmcli command</li>
	 * <li>24. Perform factory reset on the device</li>
	 * <li>25.Verify that the accountId value has been stored in persistent memory
	 * in syscfg.db file using syscfg set command and cross validate via dmcli</li>
	...
	public void testToVerifyAccountIdFromXconfRequest(Dut device) {
		...
		try {
			...
			/**
			 * Step 1 : Verify Accountid is set in syscfg.db file using syscfg get command
			 * and cross validate via dmcli
			 */
			int stepNumber = 1;
			...
			/**
			 * Step 2-4 : Verify RFC/CDL/DCM xconf query has &accountID=<accountID> in the
			 * curl request
			 */
			...
			/**
			 * Step 7 : Restart rfc-config.service to fetch the RFC settings from the mock
			 * xconf
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
			...
			/**
			 * Step 10-12 : Verify RFC/CDL/DCM xconf query has &accountID=unknown in the
			 * curl request
			 */
			...
			/**
			 * Step 14 : Restart rfc-config.service to fetch the RFC settings from the mock
			 * xconf
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
			...
			/**
			 * Step 16: Verify Accountid is set in syscfg.db file using syscfg get command
			 * and cross validate via dmcli
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
			...
			/**
			 * Step 17-19 : Verify RFC/CDL/DCM xconf query has &accountID=<accountID> in the
			 * curl request
			 */
			...
			/**
			 * Step 20 : Verify that invalid accountId with special characters is not set
			 * via webPa
			 */
			...
			/**
			 * Step 21 : Verify invalid AccountID with special characters is not allowed to
			 * set Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AccountInfo.AccountID
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
			...
			/**
			 * Step 22 : Verify that invalid accountId with above 32 chars is not set via
			 * webPa
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
	...
	/**
	 * method to retrieve AccountID from Xconf query and validate it with given
	 * parameter
	 * 
	 ...
	 * @param accountIdFromSystem response from syscfg.db file
	 * @refactor Said Hisham
	 */
	 ...
	 * <li>3. Verify that the device appends \"accountId=<account id>\" in RFC
	 * requests when the accountId has been set via WebPA</li>
	 * <li>4. Verify that the accountId value has been stored in persistent memory
	 * in the tr181 object
	 * Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AccountInfo.AccountID</li>
	 * <li>5. Verify that the device now appends \"accountId=<account id>\" in cdl
	 * xconf requests when the accountId has been set via WebPA</li>
	 * <li>6. Reboot the device</li>
	 * <li>7. Perform a webpa GET operation and verify that accountId has been set
	 * successfully</li>
	 * <li>8. Verify that the device now appends \"accountId=<account id>\" in cdl
	 * xconf requests</li>
	 * <li>9. Verify that the device now appends \"accountId=<account id>\" in dcm
	 * requests</li>
	 * </ol>
	 * 
	...
	/**
	 * Test to Verify that RFC requests contain accountId Unknown on Every
	 * powercycle
	 * 
	 * <li>PRE CONDITION 1. Reboot the device</li>
	 * <li>1. Verify RFC xconf query has &accountID=unknown in the curl request</li>
	 * <li>2. Check Account retrived from Dut by accountID RFC in
	 * /tmp/rfc_configdata.txt</li>
	 * <li>3. Restart rfc-config.service to fetch the RFC settings from the mock
	 * xconf</li>
	 * <li>4. Verify RFC xconf query &accountID=<accountID from Dut> in the curl
	 * request</li>
	 * <li>5. Restart rfc-config.service to fetch the RFC settings from the mock
	 * xconf</li>
	 * <li>6. Verify RFC xconf query &accountID=<accountID from Dut> in the
	 * subsquent xconf call</li>
	 * <li>7. Remove the AccounID by setting invaild accountId</li>
	 * <li>8. Verify Accountid is set in syscfg.db file using syscfg get
	 * command</li>
	 * <li>9. Reboot the device</li>
	 * <li>10. Verify RFC xconf query log for Account ID mismatch &
	 * accountID=unknown</li>
	...
	public void testToVerifyAccountIdFromRfcRequest(Dut device) {
		...
		try {
			...
			/**
			 * Step 2 : Check Account retrived from Dut by accountID RFC in
			 * /tmp/rfc_configdata.txt
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 3 : Restart rfc-config.service to fetch the RFC settings from the mock
			 * xconf
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 4 : Verify RFC xconf query has &accountID=<accountID> in the curl
			 * request
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 5 : Restart rfc-config.service to fetch the RFC settings from the mock
			 * xconf
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 6 : Verify RFC xconf query &accountID=<accountID from Dut> in the
			 * subsquent xconf call & http_code: 304
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			errorMessage = "accountID = " + accountId + " is not being sent in DCM xconf request ";
			...
			/**
			 * Step 7 : Set Invalid AccountID with
			 * Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AccountInfo.AccountID
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 8: Verify Accountid is set in syscfg.db file using syscfg get command
			 * and cross validate via dmcli
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
			...
			/**
			 * Step 10 : Verify RFC xconf query log for Account ID mismatch &
			 * accountID=unknown
			 */
			stepNumber++;
			stepNum = "s" + stepNumber;
