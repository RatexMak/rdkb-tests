...
public class BroadBandSystemTests extends AutomaticsTestBase {
	...
	/**
	 * Command to list files order by creation date. Most recent file will be shown
	 * first
	 */
	...
	/**
	 * Verify disk usage, up-time, top, date and disk space response from Arm
	 * Console
	 * <ol>
	 * <li>Verify uptime - To check the uptime of the Broadband Device (run uptime
	 * command in Arm Console)</li>
	 * <li>Verify date - To check the date (run date command in Arm Console)</li>
	 * <li>Verify total space and available space of the file system (run df command
	 * in Arm Console)</li>
	...
	/**
	 * 
	 * Test Case to verify [DNSMASQ version check] Unify dnsmasq version across
	 * different RDKB devices
	 * 
	 * <li>Steps:</li>
	 * <ol>
	 * <ol>
	 ...
	 * <li>EXPECTED: Running status of dnsmasq service must be in active(running)
	 * state</li>
	 * </ol>
	 * <ol>
	 ...
	 * <li>EXPECTED: Response should contain \"HTTP/1.1 200 OK\" text or HTTP/1.1
	 * 302 and "Connected to www.google.com"</li>
	 * </ol>
	 * <ol>
	...
	public void testToVerifyDnsMasqVersion(Dut device) {
		...
		try {
			...
			if (BroadbandPropertyFileHandler.isIpv6Enabled()) {
				LOGGER.info("**********************************************************************************");
				LOGGER.info("STEP " + stepNumber
						+ ": DESCRIPTION : Verify response by querying IPv6 enabled external server");
				LOGGER.info("STEP " + stepNumber
	 ...
	 * <ol>
	 * <li>Verify standard Time Stamp format for "wifihealth.txt" command in Atom
	 * Console.</li>
	 * <li>Verify standard Time Stamp format for "/usr/ccsp/wifi/aphealth.sh"
	 * command in Atom Console.</li>
	 * </ol>
	 * *
	 * 
	 * @param device {@link Dut}
	 ...
	 * 
	 * @param {@response} response retrieved after executing the command.
	 * 
	 ...
	 * <li>Steps:</li>
	 * <ol>
	 * STEP 1: Create a testfile within /nvram using /dev/urandom and copy the file
	 * to the path /opt/secure/testfile STEP 2: Repeat the above "STEP 1" for 100
	 * times and then take a diff between /nvram/testfile and /opt/secure/testfile.
	 * The response should be null STEP 3: Reboot the device STEP 4: verify the diff
	 * between /nvram/testfile /opt/secure/testfile
	 * </ol>
	 * 
	...
	/**
	 * List of servers (URIs) and minimum set of servers to ping MUST be
	 * configurable via bootfile SNMP OIDs.
	 * <ol>
	 * 
	 * <li>Verification of adding 20 Ipv4 ping server using Webpa POST command on
	 * TR-181 parameter
	 * "Device.SelfHeal.ConnectivityTest.PingServerList.IPv4PingServerTable."</li>
	 * <li>Verification of adding 21st Ipv4 ping server using Webpa POST command on
	 * TR-181 parameter *
	 * "Device.SelfHeal.ConnectivityTest.PingServerList.IPv4PingServerTable."</li>
	 * <li>Verification of adding Invalid Ipv4 ping server \"1.2.3.4\" by using
	 * Webpa POST command on TR-181 parameter
	 * "Device.SelfHeal.ConnectivityTest.PingServerList.IPv4PingServerTable"</li>
	 * <li>Verification of adding Invalid Ipv6 ping server 2001::2002\" by using
	 * Webpa POST command on TR-181 parameter
	 * "Device.SelfHeal.ConnectivityTest.PingServerList.IPv6PingServerTable"</li>
	 * <li>verify whether \"PING_FAILED:1.2.3.4\" message is present in self
	 * heallog.txt.0 after the setting the InValid servers</li>
	 * <li>verify whether \"PING_FAILED:2001::2002\" message is present in self
	 * heallog.txt.0 after the setting the InValid servers</li>
	 * </ol>
	 */
	@Test(enabled = true, dataProvider = DataProviderConstants.PARALLEL_DATA_PROVIDER, dataProviderClass = AutomaticsTapApi.class, groups = TestGroup.SYSTEM)
	 ...
	 * <li>Set Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.DNSStrictOrder.Enable
	 * is set true</li>
	 * <li>Restart the dnsmasq service and verify whether the feature got enabled or
	 * not</li>
	 * <li>Execute tcpdump command and try pinging valid url and verify
	 * /opt/dummy.txt file is created or not</li>
	 ...
	 * <li>Verify whether the DNS queries then fallback to secondary when the
	 * primary is not available</li>
	 * <li>Verify whether the DNS queries then fallback to secondary within 5
	 * sec</li>
	 * </ol>
	 * 
	 ...
	 * @param commands       - commands to be executed to check the atom access
	 * @param expectedString - expected response for command execution
	 * @param console        - console details
	 * 
	 ...
	 * <li>Verification of adding invalid special characters with usage compute
	 * window using the TR181
	 * parameter-Device.SelfHeal.ConnectivityTest.X_RDKCENTRAL-COM_UsageComputeWindow</li>
	 * <li>Verification of adding invalid value with usage compute window using the
	 * TR181
	 * parameter-Device.SelfHeal.ConnectivityTest.X_RDKCENTRAL-COM_UsageComputeWindow</li>
	...
	/**
	 * 
	 * This is to verify the encryption of /opt/secure folder. Upon unmounting the
	 * folder the contents of the folder should be encrypted. Thus it prevents the
	 * unauthorized alteration.
	 * 
	 * Common implementation for nvram init to eliminate dependencies and patches
	 * (productization)
	 * <li>Steps:</li>
	 * <ol>
	 ...
	 * </ol>
	 * <ol>
	 * STEP 15 : Verify the logs 'Mounted cpc directory succssfully' in case of
	 * successful mounting of /opt/secure
	 * </ol>
	 * <ol>
	 * STEP 16 : Restart securemount service when /opt/secure is already mounted
	 * </ol>
	 * <ol>
	 * STEP 17 : Verify that error log 'Failed to mount cpc directory' is observed
	 * on restarting securemount service when /opt/secure is already mounted
	 * </ol>
	 * <ol>
	 ...
	 * </ol>
	 * <ol>
	 * STEP 21 : Verify that previous contents of /opt/secure are no longer
	 * available
	 * </ol>
	 * 
	 * @param device instance of {@link Dut}
	...
	/**
	 * 
	 * Test Case : Verify Add IP from ARM/ATOM side private network using Ping and
	 * ip route
	 * 
	 * <ol>
	 * <li>Step 1: Verify ping 8.8.8.8 response in ARM/ATOM console</li>
	 * <li>Step 2: Verify dev interface default ip in ip route response at ARM/ATOM
	 * console</li>
	 * <li>Step 3: Verify dev interface proto kernel scope link in ip route response
	 * at ARM/ATOM console</li>
	 * <li>Step 4: Verify dev interface scope link in ip route response at ARM/ATOM
	 * console</li>
	 * </ol>
	 * 
	 * @param device {@link Dut}
	...
	public void testToVerifyPingAndIpRouteResponse(Dut device) {
		...
		try {
			...
			/**
			 * STEP 2 : Verify dev interface default ip in ip route response at ARM/ATOM
			 * console
			 */
			stepNumber++;
			step = "S" + stepNumber;
			...
			/**
			 * STEP 3 : Verify dev interface proto kernel scope link in ip route response at
			 * ARM/ATOM console
			 */
			stepNumber++;
			step = "S" + stepNumber;
	...
	/**
	 *
	 * Test Case : Verify fixes for ccsp-webpa-adapter N ,WiFi values,2.4 & 5 GHz
	 * Radios and common parameter after webpa reboot
	 *
	 * <p>
	 * STEPS:
	 * </p>
	 ...
	 * <li>Step 1 : Verify webpa process is running on ATOM side using the following
	 * command in ATOM console: ps | grep webpa</li>
	 * <li>Step 2 : Verify the SSID value using the WebPA param
	 * Device.WiFi.SSID.10001.SSID</li>
	 * <li>Step 3 : Verify the CM MAC is retrieved using the WEBPA Param:
	 * Device.DeviceInfo.X_COMCAST-COM_CM_MAC</li>
	 ...
	 * <li>Step 11 : Verify webpa process is running on ATOM side using the
	 * following command in ATOM console: ps | grep webpa</li>
	 * <li>Step 12 : Verify last reboot reason value when reboot triggered through
	 * WebPA using the following param
	 * Device.DeviceInfo.X_RDKCENTRAL-COM_LastRebootReason</li>
	 * <li>Step 13 : Verify the SSID value using the WebPA param
	 * Device.WiFi.SSID.10001.SSID</li>
	 * <li>Step 14 : Verify WebPA get for Device.IP.Interface.1.Enable with Value
	 * Either True fro Enable and False for Disable.</li>
	 * <li>Step 15 : Retrieve value for 2.4 & 5 GHz Radios from device and cross
	 * verify with values retrieved from Step #7</li>
	 * <li>Step 16 : Retrieve common parameter values from device and cross verify
	 * with values retrieved from Step #8</li>
	 * <li>Step 17 : Retrieve WAN IP and Client IP Router values from from Step
	 * #4</li>
	...
	public void testToVerifyFixesForCCSPWebpaAdapter(Dut device) {
		...
		try {
			...
			/**
			 * Step 1 : VERIFY WEBPA PROCESS IS RUNNING ON ATOM SIDE USING THE FOLLOWING
			 * COMMAND IN ATOM CONSOLE
			 */
			testStepNumber = "S" + stepNumber;
			...
			/**
			 * Step 15 : RETRIEVE AND VERIFY THE VALUE FOR 2.4 & 5 GHZ RADIOS FROM DEVICE
			 * AFTER REBOOT
			 */
			stepNumber++;
			testStepNumber = "S" + stepNumber;
			...
			/**
			 * Step 17 : RETRIEVE WAN IP AND CLIENT IP ROUTER VALUES FROM DEVICE AFTER
			 * REBOOT
			 */
			stepNumber++;
			testStepNumber = "S" + stepNumber;
	...
	/**
	 * This test case is to Verify CPU usage verify when CPU usage stays 100% for
	 * over 5 mins, top 5 processes gets logged in SelfHeal.txt.0
	 * 
	 * <ol>
	 * <li>STEP 1 : Make CPU usage as 100% using system commands</li>
	 * <li>STEP 2 : Verify CPU usage 100% reached status</li>
	 * <li>STEP 3 : Verify Avg CPU usage after 5 minutes of CPU Avg monitor window
	 * is 100</li>
	 * <li>STEP 4 : Verify when CPU usage stays 100% for over 5 mins, top 5
	 * processes gets logged in SelfHeal.txt.0</li>
	 ...
	 * 
	 * @param response - Logs obtained in SelfHeal.txt file
	 * 
	 ...
	 * <li>PRE-CONDITION 2 : Reactivate the Device.</li>
	 * <li>Step 1 : Verify parodus process is running on ARM side.</li>
	 * <li>Step 2 : Verify Triggered CDL Successful with latest firmware
	 * version.</li>
	 * <li>Step 3 : Verify old firmware version is available in WEBPAlog.txt.0
	 * log.</li>
	 * <li>Step 4 : Verify current firmware version is available in WEBPAlog.txt.0
	 * log.</li>
	 * <li>Step 5 : Verify whether webpa request 'get' is communicating with the
	 * device after firmware upgrade.</li>
	 * <li>Step 6 : Verify whether webpa request 'set' is communicating with the
	 * device after firmware upgrade.</li>
	 * <li>Step 7 : Verify Triggered CDL Successful with previous firmware
	 * version.</li>
	 * <li>POST-CONDITION 1 : Verify device is reactivated,if already not
	 * activated.</li>
	 * <li>POST-CONDITION 2 : Verify that device is upgraded with the previous
	 * firmware version in #S7, if not try CDL again.</li>
	 * 
	 * </ol>
	 * 
	 * @param device {@link Dut}
	...
	public void testToVerifyWebpaFirmwareUpgrade(Dut device) {
		...
		try {
			...
			/**
			 * SETP 5 : VERIFY WHETHER WEBPA REQUEST 'GET' IS COMMUNICATING WITH THE DEVICE
			...
			/**
			 * SETP 6 : VERIFY WHETHER WEBPA REQUEST 'SET' IS COMMUNICATING WITH THE DEVICE
			 * AFTER FIRMWARE UPGRADE
			 */
			stepNumber++;
			step = "S" + stepNumber;
	...
	/**
	 * Verify whether user is able to run user or manual script even after giving
	 * 777 permission
	 * <ol>
	 ...
	 * 
	 * STEP 1:Verify sshbanner.txt file availability from /etc/ folder STEP 2:Verify
	 * sshbanner.txt file size and file corrupted status STEP 3:Verify
	 * Legal/Compliance "ssh banner" from sshbannr.txt file STEP 4:Verify dropbear
	 * processor running status STEP 5:Verify Legal/Compliance "ssh banner" after
	 * SSH to Device. STEP 6:Verify Legal/Compliance "ssh banner" after SSH to
	 * Device in ATOM_CONSOLE.
	 * 
	 ...
	 * <ol>
	 * <li>Configure and post RFC payload data to enable swap memory when RFC config
	 * service is triggered</li>
	 ...
	 * <li>Verify the memory swap enable using ZRAM is logged in Console log
	 * file</li>
	 * <li>Verify ZRAM partitions on the device after enabling swap memory</li>
	 * <li>Configure and post RFC payload data to disable swap memory when RFC
	 * config service is triggered</li>
	 * <li>Restart RFC config service to update settings</li>
	 * <li>Verify Memory Swap has been disabled by RFC script</li>
	 * <li>Verify the gateway can be rebooted using WebPA to disable memory
	 * swap</li>
	 * <li>Verify the memory swap is still disabled after reboot</li>
	 * <li>Verify the memory swap disable using ZRAM is logged in Console log
	 * file</li>
	 * </ol>
	 * 
	 * @refactor Govardhan
	 ...
	 * 
	 * <li>1. Update RFC_CONFIG_SERVER_URL value in rfc.properties file with an
	 * invalid URL and check whether the URL updated</li>
	 * <li>2. Restart the RFC service using webpa set command</li>
	 * <li>3. Verify the features reported as NONE in dcmrfc.log file</li>
	 * <li>4. Change RFC_CONFIG_SERVER_URL value in rfc.properties file with valid
	 * MOCK server URL</li>
	 ...
	 * <li>8. Verify the feature profile reported as ACTIVE and check whether the
	 * current list matches with the earlier STAGING list</li>
	 * <li>9. Restart the RFC service using webpa set command</li>
	 * <li>10. Verify the ACTIVE profile logs and check previous ACTIVE featues list
	 * is present in the current list</li>
	 * 
	 * @author ArunKumar Jayachandran
	 * @refactor Rakesh C N
	 */
	...
	public void testToVerifyRfcLogReport(Dut device) {
		...
		try {
			LOGGER.info("################### STARTING PRE-CONFIGURATIONS ###################");
			LOGGER.info("PRE-CONDITION STEPS");
			LOGGER.info("PRE-CONDITION : DESCRIPTION : Copy rfc.properties file from /etc to /nvram directory");
			LOGGER.info("PRE-CONDITION : ACTION : Execute command: cp /etc/rfc.properties /nvram/rfc.properties");
			LOGGER.info("PRE-CONDITION : EXPECTED : rfc.properties file should be present in /nvram directory");
			...
			LOGGER.info("PRE-CONDITION : ACTUAL: Successfully copied rfc.properties file to /nvram directory");
			...
			LOGGER.info("STEP 1: ACTION: Change the value of RFC_CONFIG_SERVER_URL to an invalid url ");
