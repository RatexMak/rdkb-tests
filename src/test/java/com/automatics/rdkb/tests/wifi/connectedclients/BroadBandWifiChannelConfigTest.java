...
public class BroadBandWifiChannelConfigTest extends AutomaticsTestBase {
	...
	/**
	 * Verify Client Connectivity to 2.4GHz bands with Channel Selection Mode is
	 * Manual.
	 *
	 * <p>
	 * STEPS:
	 ...
	 * <li>S2) Verify connecting the WiFi Client in the Setup to the 2.4GHz
	 * SSID</li>
	 ...
	 * <li>S6) Verify connecting the WiFi Client in the Setup to the 2.4GHz
	 * SSID</li>
	 * <li>S7) Verify the internet connectivity in the Client after changing the
	 * channel selection mode.</li>
	 ...
	 * <li>S11) Verify connecting the WiFi Client in the Setup to the 2.4GHz SSID
	 * and start capturing WiFi Packets.</li>
	 * <li>S12) Verify the internet connectivity in the Client after changing the
	 * channel.</li>
	 ...
	 * <li>S17) Verify connecting the WiFi Client in the Setup to the 2.4GHz SSID
	 * and start capturing WiFi Packets.</li>
	 * <li>S18) Verify the internet connectivity in the Client after changing the
	 * channel.</li>
	...
	public void testChannelSelectionModeWifiRadio2Ghz(Dut device) {
		...
		try {
			...
			LOGGER.info("STEP " + stepNumber + " - ACTUAL: "
					+ (status ? "SUCCESSFULLY CONNECTED THE WI-FI CLIENT ASSOCIATED WITH THE SETTOP TO A 2.4GHz SSID"
							: errorMessage));
			...
			/**
			 * S6) Verify connecting the WiFi Client in the Setup to the 2.4 GHz SSID S7)
			 * Verify the internet connectivity in the Client after changing the channel
			 * selection mode. S8) Verify disconnecting the WiFi Client from the Gateway
			 * device.
			 */
			stepNumber++;
			...
			/**
			 * S11) Verify connecting the WiFi Client in the Setup to the 2.4GHz SSID &
			 * start capturing WiFi Packets. S12) Verify the internet connectivity in the
			 * Client after changing the channel. S13) Verify disconnecting the WiFi Client
			...
			/**
			 * S16) Verify connecting the WiFi Client in the Setup to the 2.4GHz SSID &
			 * start capturing WiFi Packets. S17) Verify the internet connectivity in the
			 * Client after changing the channel. S18) Verify disconnecting the WiFi Client
			 * from the Gateway device.
			 */
			stepNumber++;
			connectClientAndVerifyChannel(tapEnv, device, testCaseId, stepNumber, true, false);
	...
	/**
	 * Verify Client Connectivity to 5GHz bands with Channel Selection Mode is
	 * Manual.
	 *
	 * <p>
	 * STEPS:
	 ...
	 * <li>S17) Verify the internet connectivity in the Client after changing the
	 * channel.</li>
	...
	public void testChannelSelectionModeWifiRadio5Ghz(Dut device) {
		...
		try {
			...
			/**
			 * S6) Verify connecting the WiFi Client in the Setup to the 5GHz SSID S7)
			 * Verify the internet connectivity in the Client. S8) Verify disconnecting the
			 * WiFi Client from the Gateway device.
			 */
			stepNumber++;
			connectClientAndVerifyChannel(tapEnv, device, testCaseId, stepNumber, false, false);

			/**
			 * S9) Verify changing the channel to a random values from the possible channel
			 * values.
			 */
			stepNumber = stepNumber + 3;
			testStepNumber = "S" + stepNumber;
			...
			LOGGER.info("STEP " + stepNumber + " - ACTUAL: "
					+ (status ? "VERIFIED THE CHANNEL VALUE (5GHz) SET SUCCESSFULLY TO " + channelNumber
							: errorMessage));
			tapEnv.updateExecutionStatus(device, testCaseId, testStepNumber, status, errorMessage, true);

			/**
			 * S11) Verify connecting the WiFi Client in the Setup to the 5GHz SSID & start
			 * capturing WiFi Packets. S12) Verify the internet connectivity in the Client
			 * after changing the channel. S13) Verify disconnecting the WiFi Client from
			 * the Gateway device.
			 */
			stepNumber++;
			connectClientAndVerifyChannel(tapEnv, device, testCaseId, stepNumber, false, false);
			...
			/**
			 * S14) Verify changing the channel to a random values from the possible channel
			 * values (other than the previous one).
			 */
			stepNumber = stepNumber + 3;
			testStepNumber = "S" + stepNumber;
			status = false;
			...
			/**
			 * S16) Verify connecting the WiFi Client in the Setup to the 5GHz SSID & start
			 * capturing WiFi Packets. S17) Verify the internet connectivity in the Client
			 * after changing the channel. S18) Verify disconnecting the WiFi Client from
			 * the Gateway device.
			 */
			stepNumber++;
			connectClientAndVerifyChannel(tapEnv, device, testCaseId, stepNumber, false, false);
	...
	/**
	 * Helper Method to execute the test steps which are common across various test
	 * cases. It covers the test steps that are getting repeated; it includes
	 * connecting the wifi client, packet capture, verify internet connectivity, and
	 * disconnect the wifi client.
	 * 
	 * @param tapEnv             {@link AutomaticsTapApi}
	 * @param device             {@link Dut}
	 * @param testCaseId         String representing the Test Case ID.
	 * @param stepNumber         Integer representing the step number.
	 * @param verify2GhzRadio    Boolean flag representing the verification radio,
	 *                           TRUE for 2.4GHz; FALSE for 5GHz.
	 * @param captureWiFiPackets Boolean flag representing captureWiFiPackets needed
	 *                           or not.
	 * @refactor Athira
	 */
	private void connectClientAndVerifyChannel(AutomaticsTapApi tapEnv, Dut device, String testCaseId, int stepNumber,
		...
		try {
			/**
			 * Verify connecting the WiFi Client in the Setup to the 2.4GHz/5GHz SSID &
			 * start capturing WiFi Packets as required.
			 */
			LOGGER.info("#######################################################################################");
			...
			// Connect to WiFi Client. In case, WiFi Packet capture is required, then the
	 ...
	 * 
	 * @param device                      {@link Dut}
	 * @param ethernetClient              Dut object representing the Ethernet
	 *                                    Client
	 * @param is2GhzRadio                 Boolean flag representing the 2.4GHz/5GHz
	 *                                    Radio; TRUE if 2.4GHz Radio; else FALSE
	 * @param startedCapturingWiFiPackets Boolean representing the flag whether
	...
	/**
	 * Helper Method to retrieve the channel number in Random manner. It retrieves
	 * the possible channel list; then the current channel number; then retrieves
	 * the element index randomly to return the channel number.
	 * 
	 * @param tapEnv           {@link AutomaticsTapApi}
	 * @param device           {@link Dut}
	 * @param isChannelFor5Ghz Boolean representing the 2.4GHz or 5GHz WiFi Band
	 * 
