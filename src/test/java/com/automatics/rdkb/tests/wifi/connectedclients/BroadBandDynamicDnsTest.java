...
public class BroadBandDynamicDnsTest extends AutomaticsTestBase {
	...
	try {
	    ...
	    /**
	     * PRECONDITION 1-5 : connecting client setup to 2.4 and verifying internet connectivity using IPV4 and IPV6
	     * interface
	     */
	...
	try {
	    ...
	    LOGGER.info("PRE-CONDITION STEPS");
	    /**
	     * PRECONDITION 1-5 : connecting client setup to 5 GHz and verifying internet connectivity using IPV4 and
	     * IPV6 interface
	     */
	...
	LOGGER.info("STEP " + stepNumber + ": ACTION : EXECUTE COMMAND: NSLOOKUP -TYPE=TXT WHICH.OPENDNS.COM");
    ...
    /**
     *
     * Test Case : Verifying the alternate DNS nslookup functionality for Ethernet
     * <p>
     * STEPS:
     * </p>
     * <ol>
     ...
     * <li>PRE-CONDITION 4 : Verify the internet connectivity in the client connected with ethernet using ipv4 interface
     * </li>
     * <li>PRE-CONDITION 5 : Verify the internet connectivity in the client connected with ethernet using ipv6 interface
     * </li>
     ...
     */
    @Test(dataProvider = DataProviderConstants.CONNECTED_CLIENTS_DATA_PROVIDER, dataProviderClass = AutomaticsTapApi.class, alwaysRun = true, enabled = true)
    @TestDetails(testUID = "TC-RDKB-LAN-DNS-NSLOOKUP-5001")
    public void testToVerifyAlternateDnsNSlookupOnLAN(Dut device) {

	Dut deviceConnectedWithLAN = null;
	String testCaseId = "TC-RDKB-LAN-DNS-NSLOOKUP-501";
	int stepNumber = 1;
	stepNum = "S" + stepNumber;
	errorMessage = null;
	status = false;
	LOGGER.info("#######################################################################################");
	LOGGER.info("STARTING TEST CASE: TC-RDKB-LAN-DNS-NSLOOKUP-5001");
	LOGGER.info("TEST DESCRIPTION: Verifying the alternate DNS nslookup functionality for Ethernet");
	LOGGER.info("TEST STEPS : ");
	LOGGER.info("PRE-CONDITION 1 : Connect the client setup to ethernet SSID and verify connection status");
	...
	LOGGER.info(
		"PRE-CONDITION 4 : Verify the internet connectivity in the client connected with ethernet using ipv4 interface");
	LOGGER.info(
		"PRE-CONDITION 5 : Verify the internet connectivity in the client connected with ethernet using ipv6 interface");
	...
	try {
	    ...
	    LOGGER.info("PRE-CONDITION STEPS");
	    /**
	     * PRECONDITION 1-5 : connecting client setup to Ethernet and verifying internet connectivity using IPV4 and
	     * IPV6 interface
	     */
	    deviceConnectedWithLAN = BroadBandPreConditionUtils.executePreConditionToVerifyLanClientStatus(device,
		    tapEnv);
	    LOGGER.info("################### COMPLETED PRE-CONFIGURATIONS ###################");
	    verifyDnsNsLookupFunctionality(device, deviceConnectedWithLAN, testCaseId);
	} catch (Exception e) {
	    errorMessage = e.getMessage();
	    LOGGER.error("EXCEPTION OCCURRED WHILE VERIFYING THE ALTERNATE DNS NSLOOKUP FUNCTIONALITY FOR ETHERNET"
		    + errorMessage);
	    CommonUtils.updateTestStatusDuringException(tapEnv, device, testCaseId, stepNum, status, errorMessage,
		    true);
	}
	LOGGER.info("ENDING TEST CASE: TC-RDKB-LAN-DNS-NSLOOKUP-5001");
    }
