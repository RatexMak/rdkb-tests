...
public class BroadBandMsoWebGuiTest extends BroadBandWebUiBaseTest {
	 ...
	 *         true if all private ssid is not enabled through SNMP/TR-069/WebPA
	 * @author Praveenkumar Paneerselvam
	 */
	public BroadBandResultObject verifyEnablingPrivateSsid(Dut device, SSIDFrequency ssidFrequency) {
		...
		if (BroadbandPropertyFileHandler.isTr69Enabled()) {
			...
			if (!DeviceModeHandler.isBusinessClassDevice(device)) {
				...
				// NA
				LOGGER.info(
						"Status of enable Private SSID through TR-69 is NA for Business Class Devices" + tr69Status);
			}
		} else {
			LOGGER.info("TR-69 is disabled");
			tr69Status = false;
		}
		// 3.Through WEBPA
		...
		if (BroadbandPropertyFileHandler.isTr69Enabled()) {
			...
			executionStatus.setStatus(snmpStatus && webpaStatus);
	 ...
	 *              <li>PRE-CONDITION-2 : Set and verify the Global DNS IPv6 using
	 *              webpa param
	 ...
	 * <ol>
	 * <li>Retrive the default values of MFPConfig</li>
	 * <li>Set the values of MFPConfig to \"Optional\" and verify the same via Wi-Fi
	 * HAL</li>
	 * <li>Set the values of MFPConfig to \"Required\" and verify the same via Wi-Fi
	 * HAL</li>
	 ...
	 * <ol>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.Queries is not
	 * writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.QueryRate is not
	 * writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.Responses is not
	 * writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.ResponseRate is not
	 * writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.NoRequestOutstanding is
	 * not writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.ResponsesDiscarded is not
	 * writable</li>
	 * <li>Verify Device.WiFi.X_RDKCENTRAL-COM_GASStats.1.FailedResponses is not
	 * writable</li>
	 ...
	 * <li>Enable ANQP and Passpoint debugging for WiFi (on atom for Atomsync
	 * Devices)</li>
	 ...
	 * <li>Step 3 : Verify the CcspXdnsSsp process is up and running on the
	 * device</li>
	 * <li>Step 4 : kill and verify the CcspXdnsSsp process</li>
	 * <li>Step 5 : Verify log 'Resetting process CcspXdnsSsp' is available in
	 * /rdklogs/logs/SelfHeal.txt.0. For DSL log should be in
	 * systemd_processRestart.log</li> *
	...
	public void testToVerifyXDNSProcessAfterReboot(Dut device) {
		...
		isSystemdPlatforms = BroadbandPropertyFileHandler.isDeviceCheckForXDNS(device)
				|| (DeviceModeHandler.isDSLDevice(device)) || DeviceModeHandler.isFibreDevice(device);
		int stepNumber = 1;
		stepNum = "S" + stepNumber;
		try {
			LOGGER.info("#######################################################################################");
			...
			float latestUpnpVersion = Float
					.parseFloat(BroadbandPropertyFileHandler.getValueForCurrentUPNPVersion(device));
			if (CommonMethods.isNotNull(response)) {
			...
			/**
			 * Step 5 : Verify the CcspXdnsSsp process available in
			 * /rdklogs/logs/systemd_processRestart.log for arm and DSL devices and for
			 * other devices it is available in /rdklogs/logs/SelfHeal.txt.0 .
			 */
			stepNumber++;
			stepNum = "S" + stepNumber;
			status = false;
	...
	/**
	 * Test to validate the private 24/5ghz_Verify BSSID remains same after Factory
	 * Reset
	 * 
	 ...
	 * 
	 * @param device Instance {@link Dut}
	 * 
	 * @author Sathurya_R
	 * @Refactor Rakesh C N
	 * 
